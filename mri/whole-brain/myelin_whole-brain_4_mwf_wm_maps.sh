...
echo "##########################################################################"
echo "                MWF Analysis Pipeline - Whole-brain WM (4)                "
echo "##########################################################################"
...
do
	...
	fslstats $subj/$timepoint/${subj}_${visit}_MWF_map_in_GRASE.nii.gz -k $subj/$timepoint/grase_wm/${subj}_wm_mask_in_grase_thr1_done -m -s -v > $subj/$timepoint/mwf_wm/MWF_TOT_WM_map.txt
	...
	awk '{OFS=", "; print $1,$2, $3, FILENAME}' $subj/$timepoint/mwf_wm/MWF_TOT_WM_map.txt \
...
do
	...
	awk '{gsub("_map.txt",""); print}' $subj/$timepoint/mwf_wm/${subj}_MWF_TOT_WM_map_results_3_temp.csv \
		> $subj/$timepoint/mwf_wm/${subj}_MWF_TOT_WM_map_results.csv
done
...
do
	...
	rm $subj/$timepoint/mwf_wm/*WM_map.txt
